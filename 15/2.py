...
if __name__ == '__main__':
    # increase the power of elves starting from 4
    # Check if the number of elves who start the game are the same when they end the game as well
    # increase until...

    # file_name = "test2.in"
    file_name = "1.in"
    board,units = create_board(file_name)
    elf_count = len(list(filter(lambda x: x.type == UnitType.Elf, units)))
    elf_count_after_battle = 0
    elf_attack_power = 3
    while elf_count != elf_count_after_battle:
        board,units = create_board(file_name)

        elf_attack_power += 1
        for elf in filter(lambda x: x.type == UnitType.Elf, units):
        ...
        elf_count_after_battle = len(list(filter(lambda x: x.type == UnitType.Elf, units)))
